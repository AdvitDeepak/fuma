...
Plots difference in attack performance vs. number of epochs unlearned.
...
n = 5
modes = ["easy", "hard"]
attacks = ["attack_grad-smart", "attack_loss-smart", "expmt_grad-ratio", "expmt_grad-ratio-fast"]
datasets = ["tofu", "rwku"]
attack = attacks[3]
mode = modes[1]
...
# Load in results
with open(f"epochs_{attack}_{n}_{mode}.json", "r") as f:
    saved = json.load(f)
...
for entry in results:
    ...
    if epochs not in res: 
        res[epochs] = vals
    else: 
        for k in vals:
            res[epochs][k].append(vals[k][0])
...
# Recall@3
ax.plot(x, margin_mean, marker='x', markersize=6, linewidth=2, label=f'Margin% - {attack_name}', color=palette[3])
ax.fill_between(x, np.array(margin_mean) - np.array(margin_std), np.array(margin_mean) + np.array(margin_std),
                color=palette[3], alpha=0.1)


ax.set_xlabel("Number of Epochs Unlearned", fontsize=14)
ax.set_ylabel("Score (%)", fontsize=14)
ax.set_title(f"Attack Success vs. Unlearning Duration", fontsize=15)
...
plt.tight_layout()
plt.savefig(f"plots/expmt_epochs_{attack}_{mode}.png", bbox_inches='tight', dpi=300)
