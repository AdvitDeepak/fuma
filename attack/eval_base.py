...
Evaluate attack methods (generic version).
...
def evaluate_attack(rows, prediction_function, tokenizer, existing_results, result_path):
    ...
    for idx, row in enumerate(tqdm(rows, desc="Evaluating")):
        ...
        if ds == "tofu":
            ...
            if Constants.MODE == "easy":
                remaining_indices = list(set(all_rwku_indices) - base_indices)
            else:
                idx = next(iter(base_indices))
                target_subject = rwku_dataset[idx]["subject"]
                remaining_indices = [
                    i for i, entry in enumerate(rwku_dataset)
                    if entry["subject"] == target_subject and i not in base_indices
                ]

            extra_indices = random.sample(remaining_indices, min(len(remaining_indices), Constants.N - 1))
            extra_questions = [rwku_dataset[i]["query"] for i in extra_indices]
...
def main():
    ...
    result_path = f"results/{args.attack}_{Constants.N}_{Constants.MODE}.json"
    ...
    with open(f"results/{args.attack}_{Constants.N}_{Constants.MODE}.json", "r") as f:
        saved = json.load(f)
    ...
    with open(f"results/{args.attack}_{Constants.N}_{Constants.MODE}.json", "w") as f:
