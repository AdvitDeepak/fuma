...
Evaluate attack methods on increasing LoRA rank size.
...
def get_valid_rows_for_multi(rows):
    ...
    for need_ix in need_ixs: 
        for row in rows:
            ...
            if row["indices"] == need_ix and row["LoRA-rank"] == 8 and row["dataset"] == "tofu":
                valid_rows.append(row)
                total += 1
                break 
...
def main():
    ...
    result_path = f"results/ranks_{args.attack}_{Constants.N}_{Constants.MODE}.json"
